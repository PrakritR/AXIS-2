...
| Manager account creation ("Get started") | `docs/agents/manager-account-creation.md` | `/auth/create-account` NEVER auto-redirects to a portal — a signed-in user who already holds the tab's role gets the shared already-have-access panel (`AuthAlreadyHaveRolePanel` + `useSignedInPortalRoles`: explicit "Go to your portal" click, with a "Create a different account" reveal on manager/vendor), any other signed-in user still gets the full create form, and the partner-pricing OAuth callback returns there on every branch (free tier included, `account_ready=1` when provisioned) instead of resolving a portal path. Entering a portal is always an explicit click. The email/password form must send `fullName` + `phone`; `/api/auth/manager-register` 400s without them. |
| Applicant ID / income photos | `docs/agents/application-id-photos.md` | Photos live in the PRIVATE `application-documents` bucket, referenced only by the object path stored on `row_data.application` (so they persist/resume like any answer, never base64). Reads stream through `/api/portal/application-photos`, whose `canActorAccessApplicationPhoto` re-authorizes EVERY request (owning manager by property access, applicant by session email, admin) — one manager can never reach another's applicants' photos. Uploads go direct-to-Storage via short-lived signed upload URLs minted by the same route after `authorizeApplicationPhotoWrite` (guests authorize ONLY by the row's resident-setup token, never a claimed email; no row → no mint; decided rows are immutable except to admin; quota + MIME/size caps + per-IP rate limit on the mint). Extend `sanitizeApplicationFormForListing`, never bypass it, when a variant must not carry a photo. |
...
| Multiple resident applications (one per property/room) | this section | A resident may hold several in-progress applications at once — one per (property, room-or-bundle), never one per person. `ApplicationRequestTarget` + `targetMatchesApplication` (`src/lib/rental-application/in-progress-application.ts`) are the ONE comparator for "is this apply request the SAME application as that existing one" — asymmetric on purpose: a request naming no room/bundle matches any in-progress draft for that property, but a request naming one only matches that exact room/bundle. Both `resident-applications-panel.tsx` (which in-progress row to show/resume) and `rental-application-wizard.tsx` (which draft to load) use it; do not reintroduce a plain string-key match, it can't express that asymmetry. The wizard gates its sync-to-server effect behind a `reconciledSignature` that requires a saved axis id (`loadRentalWizardDraftAxisId()`), not just a draft that happens to match — a fresh, unsaved local draft auto-populated from the URL looks like a match too, and trusting it skips the server lookup that finds (or rules out) a real existing application, creating a duplicate on reload. The resident Applications page's top-right "Apply to a property" button opens an IN-PLACE searchable property picker (`resident-applications-panel.tsx` hydrates the public catalog on open via `loadPublicExtraListingsFromServer` — the resident portal doesn't otherwise load it), then navigates to `/resident/applications/apply?propertyId=…` so the wizard opens inline under a new row in the same list; the picker's secondary "Browse homes" still goes through `residentBrowseFromApplicationHref()` to `/rent/browse`. Either way, never straight into `/resident/applications/apply` with no id. **Apply-mode auto-expand is target-bound**: the panel auto-opens ONLY the row matching the URL's apply target (or the sole in-progress draft on a bare `/apply`), once per resolved id (`autoExpandedApplyIdRef`), and the embedded wizard renders only under that row (`wizardRowId`, distinct from the click-following `expandedId`) — never `inProgress[0]` and never re-snapping on a background sync tick, which is what hijacked clicks on every other row so withdraw/edit could hit the WRONG application. With several drafts and no target it opens none. Coverage: `tests/unit/resident-applications-row-identity.test.tsx`. **Withdrawal is FINAL for the applicant** (captain decision, Jul 2026 — Option A): `findInProgressRowForTarget` excludes withdrawn rows and `confirmWithdraw` (`resident-applications-panel.tsx`) cancels the queued autosave and clears the local wizard draft + axis id, so reapplying to the same property mints a brand-new application; the manager keeps the withdrawn record (withdraw stamps `withdrawnAt`, never deletes). Withdraw authorization is per-record EMAIL ownership (`rowEmail === email` in the route), never a portal-role gate — a multi-role account (primary `profiles.role` manager/owner who also applied as a resident) owns any application carrying its own email, exactly the match the list uses; a manager withdrawing someone ELSE's application still fails that check and uses Reject. Do not reintroduce a sticky `withdrawnAt` merge stamp — it would permanently hide the reapplied row (rationale in `mergeApplicationRow`, `manager-applications-storage.ts`). Coverage: `tests/unit/in-progress-application.test.ts`, `tests/unit/manager-applications-merge-rows.test.ts`, `tests/unit/manager-applications-withdraw-multi-role.test.ts`. |
