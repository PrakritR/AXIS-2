...
| Applicant ID / income photos | `docs/agents/application-id-photos.md` | Photos live in the PRIVATE `application-documents` bucket, referenced only by the object path stored on `row_data.application` (so they persist/resume like any answer, never base64). All bytes flow through `/api/portal/application-photos`, whose `canActorAccessApplicationPhoto` re-authorizes EVERY request (owning manager by property access, applicant by email, admin) — one manager can never reach another's applicants' photos. Extend `sanitizeApplicationFormForListing`, never bypass it, when a variant must not carry a photo. |
