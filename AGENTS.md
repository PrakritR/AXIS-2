...
| Resident account creation | `docs/agents/resident-onboarding.md` | A prospective resident CREATES AN ACCOUNT then APPLIES FROM INSIDE THEIR PORTAL (captain decision, Jul 2026). `POST /api/auth/resident-register` is ENABLED — self-serve email/phone/password signup via the safe `provisionResidentAccountByEmail` (service-role; a new applicant gets `role=resident`, `application_approved=false`, no elevated role — the manager still approves). The public apply gate (`public-apply-account-prompt.tsx`) offers Create account (primary) + Sign in + guest; Create account carries `next=/rent/apply?propertyId=…` so signup lands the renter ON that application (not a bare portal — `ResidentSignupForm` navigates straight to `next`, bypassing the post-auth resolver which would drop the propertyId). The emailed setup token / in-session handoff is now the GUEST fallback only (`register-resident` legacy + `resident-setup` token flow stay 403/token-gated). Phone required. A mismatched Google email RELINKS the application, never rejects. |
