...
`/api/reports/resident-ledger` is still live (resident Documents → Rent receipts). `/api/reports/resident-balance` lost its only UI caller with the Summary tab and is orphaned-but-kept on purpose.
