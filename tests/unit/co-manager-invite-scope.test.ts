...
describe("findPropertyIdsNotOwnedByManager", () => {
  ...
  it("fails closed when ownership cannot be established, and says so in the log", async () => {
    ...
    try {
      ...
      // A silent fail-closed here reads as "co-manager invites mysteriously
      // stopped working" with nothing to correlate the outage against.
      expect(logged).toHaveBeenCalledWith(expect.stringContaining("ownership lookup failed"), "connection reset");
    } finally {
      logged.mockRestore();
    }
  });
