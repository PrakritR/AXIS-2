...
describe("portal-sandbox-accounts", () => {
  it("flags @axis.local and @test.proplane.local emails", () => {
    expect(isPortalSandboxEmail("alex.morgan@axis.local")).toBe(true);
    expect(isPortalSandboxEmail("manager@test.proplane.local")).toBe(true);
  ...
  it("blocks links between sandbox and real portal accounts", () => {
    ...
    expect(isCrossSandboxPortalPair("alex.morgan@axis.local", "manager@test.proplane.local")).toBe(false);
