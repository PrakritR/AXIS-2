 ...
 *
 * Copies listing media URLs from 5257 Brooklyn Ave NE onto 5259 Brooklyn Ave NE
 * (Ambika Mago). Does not change rent, room copy, leases, status, or move-in text.
 *
 * Dry run (default):
 *   npx tsx --env-file=.env.production.local scripts/copy-brooklyn-5257-photos-to-5259-production.ts
 *
 * Apply (captain-approved production write):
 *   ALLOW_PRODUCTION_LISTING_WRITE=1 COPY_BROOKLYN_5257_TO_5259_CONFIRM=1 \
 *   npx tsx --env-file=.env.production.local scripts/copy-brooklyn-5257-photos-to-5259-production.ts --apply
 */
import { createClient } from "@supabase/supabase-js";
import { copyListingMediaBetweenSubmissions } from "../src/lib/listing-media-copy";
import {
...
function assertProductionGate(url: string, apply: boolean) {
  ...
  if (apply) {
    ...
    if (process.env.COPY_BROOKLYN_5257_TO_5259_CONFIRM !== "1") {
      console.error("Set COPY_BROOKLYN_5257_TO_5259_CONFIRM=1 to apply this one-time copy.");
      process.exit(1);
    }
  }
}

function mediaFingerprint(sub: ManagerListingSubmissionV1 | null): string {
  if (!sub) return "no-submission";
  const roomPhotos = sub.rooms.map((r) => `${r.name}:${r.photoDataUrls.length}`).join("|");
  const sharedPhotos = sub.sharedSpaces.map((s) => `${s.name}:${s.photoDataUrls.length}`).join("|");
  return `house=${sub.housePhotoDataUrls.length} rooms=[${roomPhotos}] shared=[${sharedPhotos}]`;
}
...
async function main() {
  ...
  console.log("\nApplied — only listing media URLs on 5259 were updated.");
}
