...
export function vendorInviteSubject(managerName: string): string {
  ...
  return `${name} invited you to Axis as a vendor`;
}
...
export function buildVendorInviteEmailBody(params: {
  ...
  const lines = [
    greeting,
    "",
    `${managerName} invited you to join Axis as a vendor so you can see work orders offered to you, track scheduled visits, and message them directly.`,
    "",
...
<p style="margin:0 0 16px 0"><strong>${managerName}</strong> invited you to join Axis as a vendor so you can see work orders offered to you, track scheduled visits, and message them directly.</p>
...
<a href="${href}" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:14px 28px;font-size:15px;font-weight:600;color:#ffffff;text-decoration:none">Create your vendor account</a>
</td></tr></table>
