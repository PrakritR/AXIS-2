...
function toJson(rule: RuleRecord): VendorAvailabilityRule {
  ...
  return { id: rule.id, kind: "block", specificDate: rule.specific_date as string, startMinute: rule.start_minute, endMinute: rule.end_minute, note: rule.note };
}
...
export async function POST(req: Request) {
  try {
    ...
    const body = (await req.json().catch(() => ({}))) as {
      action?: "upsert-weekly" | "upsert-block" | "delete";
      id?: string;
      weekday?: number;
      specificDate?: string;
      startMinute?: number;
