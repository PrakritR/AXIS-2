...
export async function POST(req: Request) {
  try {
    ...
    const { data: senderRoleProfile } = await db.from("profiles").select("role").eq("id", user.id).maybeSingle();
    const senderRole = String(senderRoleProfile?.role ?? "").trim().toLowerCase();
    const senderIsStaff =
