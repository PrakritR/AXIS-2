"use client";

import { authCallbackUrl } from "@/lib/auth/oauth-redirect";
import { managerOauthFinishPath } from "@/lib/auth/manager-oauth-finish-path";
import { resolveBrowserAppOrigin } from "@/lib/auth/password-reset-url";
import type { PlanTierId } from "@/data/manager-plan-tiers";
import { createSupabaseBrowserClient } from "@/lib/supabase/browser";
import { useState } from "react";

export function ManagerGoogleSignupButton({
  tier,
  billing,
  fullName,
  email,
  phone,
  promo,
  discountPercent,
  disabled = false,
  onError,
}: {
  tier: PlanTierId;
  billing: "monthly" | "annual";
  fullName: string;
  email: string;
  phone?: string;
  promo?: string;
  discountPercent?: number;
  disabled?: boolean;
  onError: (message: string) => void;
}) {
  const [busy, setBusy] = useState(false);

  const startGoogleSignup = async () => {
    if (!email.trim() || !fullName.trim()) {
      onError("Enter your full name and email first.");
      return;
    }
    setBusy(true);
    try {
      const res = await fetch("/api/manager/signup-intent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          tier,
          billing,
          email: email.trim(),
          fullName: fullName.trim(),
          phone: phone?.trim() ?? "",
          promo,
          discountPercent,
        }),
      });
      const payload = (await res.json()) as { sessionId?: string; error?: string };
      if (!res.ok) {
        onError(payload.error ?? "Could not start signup.");
        setBusy(false);
        return;
      }
      if (!payload.sessionId) {
        onError("Unexpected signup response.");
        setBusy(false);
        return;
      }

      const supabase = createSupabaseBrowserClient();
      const redirectTo = authCallbackUrl(resolveBrowserAppOrigin(), managerOauthFinishPath(payload.sessionId));
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo,
          queryParams: { prompt: "select_account" },
        },
      });
      if (error) {
        onError(error.message);
        setBusy(false);
        return;
      }
      if (data?.url) {
        window.location.assign(data.url);
        return;
      }
      onError("Could not start Google sign-in.");
      setBusy(false);
    } catch {
      onError("Network error.");
      setBusy(false);
    }
  };

  return (
    <button
      type="button"
      disabled={disabled || busy}
      onClick={() => void startGoogleSignup()}
      className="flex w-full items-center justify-center gap-3 rounded-full border border-border bg-card px-4 py-3 text-sm font-semibold text-foreground shadow-[var(--shadow-sm)] transition hover:bg-accent/60 disabled:cursor-not-allowed disabled:opacity-60"
    >
      <GoogleGlyph />
      {busy ? "Redirecting…" : "Continue with Google"}
    </button>
  );
}

function GoogleGlyph() {
  return (
    <svg className="h-5 w-5 shrink-0" viewBox="0 0 24 24" aria-hidden>
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}
