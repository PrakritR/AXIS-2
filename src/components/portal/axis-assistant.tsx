...
const AxisAssistantContext = createContext<AxisAssistantContextValue | null>(null);

function useAxisAssistant() {
  return useContext(AxisAssistantContext);
}

/** True when the layout wraps children in {@link AxisAssistant}. */
export function useHasAxisAssistant() {
  return useContext(AxisAssistantContext) !== null;
}
...
export function AxisAssistant({
  ...
  // Focus the composer when the panel opens (web only — native auto-focus opens the
  // keyboard immediately and clips the composer before the user can read suggestions).
  useEffect(() => {
    if (!open || showNativeChrome) return;
    requestAnimationFrame(() => inputRef.current?.focus());
  }, [open, showNativeChrome]);
  ...
  const contextValue: AxisAssistantContextValue = { open, openAssistant };

  return (
    <AxisAssistantContext.Provider value={contextValue}>
      {children}
      <AxisAssistantFixedTrigger />
      {isClient && panel ? createPortal(panel, document.body) : null}
    </AxisAssistantContext.Provider>
  );
}
