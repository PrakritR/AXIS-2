      ...
      <nav className="flex min-h-0 flex-1 flex-col overflow-hidden px-3 py-4">
            ...
            return (
              ...
              >
                <span className="flex min-w-0 flex-1 items-center gap-2.5">
                  ...
                  <span className={`min-w-0 truncate ${active && !adminNavIcons ? "pl-2" : ""}`}>{s.label}</span>
                </span>
        ...
        {hasSignOut ? (
          <div className="mt-auto space-y-0.5 border-t border-border pt-3">
            ...
            <Link
              href="/auth/sign-in"
              className="block rounded-[14px] px-3 py-2.5 text-sm font-medium text-muted transition hover:bg-accent/70 hover:text-foreground"
            >
              Sign out
            </Link>
          </div>
          ...
          <>
            ...
            <div className="fixed right-0 bottom-0 left-0 z-[100] max-h-[min(70vh,28rem)] overflow-y-auto rounded-t-[1.35rem] border border-border bg-background px-4 pt-4 pb-[max(1.25rem,env(safe-area-inset-bottom,0px))] shadow-[0_-24px_48px_-20px_rgba(15,23,42,0.28)] lg:hidden">
              ...
              <div className="mt-3 space-y-1 border-t border-border pt-3">
                ...
                <Link
                  href="/auth/sign-in"
                  className="block rounded-[14px] px-3 py-3 text-center text-sm font-semibold text-foreground ring-1 ring-border transition hover:bg-accent/70"
                  onClick={() => setAccountOpen(false)}
                >
                  Sign out
                </Link>
