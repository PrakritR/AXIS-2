...
const VENDOR_DISPATCH_CATEGORY_OPTIONS: { value: WorkOrderCategory; label: string }[] = [
  { value: "plumbing", label: "Plumbing" },
  { value: "electrical", label: "Electrical" },
  { value: "hvac", label: "HVAC" },
  { value: "appliance", label: "Appliance" },
  { value: "access", label: "Access / Locks" },
  { value: "general", label: "General maintenance" },
  { value: "cleaning", label: "Cleaning" },
];

/** Collapsed-by-default settings card for AI-driven vendor dispatch, self-contained
 * (own GET on mount + PATCH on save) since it isn't handed settings via props. */
export function VendorDispatchSettingsPanel() {
  const { showToast } = useAppUi();
  const [settings, setSettings] = useState<VendorDispatchSettings>(DEFAULT_VENDOR_DISPATCH_SETTINGS);
  ...
  useEffect(() => {
    (async () => {
      if (isDemoModeActive()) {
        setSettings(DEFAULT_VENDOR_DISPATCH_SETTINGS);
        setLoading(false);
        return;
      }
      try {
        ...
        if (!res.ok) return;
        const body = (await res.json()) as { vendorDispatch?: VendorDispatchSettings };
        if (body.vendorDispatch) setSettings(body.vendorDispatch);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const save = async () => {
    if (isDemoModeActive()) {
      showToast("Demo mode: settings can't be saved.");
      return;
    }
    setBusy(true);
    try {
      const res = await fetch("/api/portal/automation-settings", {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ vendorDispatch: settings }),
      });
      if (!res.ok) {
        const payload = (await res.json()) as { error?: string };
        throw new Error(payload.error ?? "Could not save settings.");
      ...
      const body = (await res.json()) as { vendorDispatch: VendorDispatchSettings };
      setSettings(body.vendorDispatch);
      showToast("Vendor dispatch settings saved.");
    } catch (e) {
      showToast(e instanceof Error ? e.message : "Could not save settings.");
    } finally {
      setBusy(false);
  ...
  const toggleVendor = (vendorId: string, checked: boolean) => {
    setSettings((prev) => {
      const current = prev.approvedVendorIds ?? [];
      return {
        ...prev,
        approvedVendorIds: checked ? [...current, vendorId] : current.filter((id) => id !== vendorId),
      };
    });
  };

  const toggleCategory = (category: WorkOrderCategory, checked: boolean) => {
    setSettings((prev) => {
  ...
  return (
    <PortalCollapsibleSection title="AI vendor dispatch" defaultExpanded={false}>
      <div className="space-y-4">
        <div>
          ...
          <div className="mt-2 space-y-2">
            <label className="flex items-center gap-2 text-sm">
              <input
                type="radio"
                name="vendor-dispatch-mode"
                checked={settings.mode === "off"}
                onChange={() => setSettings((s) => ({ ...s, mode: "off" }))}
                disabled={disabled}
              />
              Off
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="radio"
                name="vendor-dispatch-mode"
                checked={settings.mode === "approve"}
                onChange={() => setSettings((s) => ({ ...s, mode: "approve" }))}
                disabled={disabled}
              />
              Suggest and ask me
            </label>
            ...
            {settings.mode === "auto" ? (
              <p className="pl-6 text-[11px] text-muted">
                Axis assigns the best-matched vendor as soon as a resident files a request, within the limits below.
              </p>
            ) : null}
          </div>
        </div>
        ...
        <div>
          <label className="flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={settings.agentMessagingEnabled}
              onChange={(e) => setSettings((s) => ({ ...s, agentMessagingEnabled: e.target.checked }))}
              disabled={disabled}
            />
            24/7 vendor messaging agent
          </label>
          <p className="mt-1 text-[11px] text-muted">
            Axis texts and messages the assigned vendor and answers their questions about the job.
          </p>
        </div>
        ...
        <div>
          <label className="flex flex-col gap-1 text-xs font-semibold text-muted">
            ...
            <Input
              ...
              value={settings.spendCapCents != null ? String(settings.spendCapCents / 100) : ""}
              onChange={(e) => {
                const raw = e.target.value;
                const amt = parseMoneyAmount(raw);
                setSettings((s) => ({ ...s, spendCapCents: raw.trim() && amt > 0 ? Math.round(amt * 100) : null }));
              }}
              disabled={disabled}
            />
          </label>
          <p className="mt-1 text-[11px] text-muted">
            Applies only when a request already has a cost estimate. Dispatch itself never commits money, vendor
            bids still need your approval.
          </p>
        </div>

        <div>
          <p className="text-xs font-semibold text-muted">Approved vendors</p>
          <label className="mt-2 flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={settings.approvedVendorIds == null}
              onChange={(e) => setSettings((s) => ({ ...s, approvedVendorIds: e.target.checked ? null : [] }))}
              disabled={disabled}
            />
            Any matched vendor
          </label>
          {settings.approvedVendorIds != null ? (
            <div className="mt-2 space-y-1.5 pl-6">
              {vendors.length === 0 ? (
                <p className="text-xs text-muted">No active vendors yet.</p>
              ) : (
                vendors.map((v) => (
                  <label key={v.id} className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={settings.approvedVendorIds?.includes(v.id) ?? false}
        ...
        <div>
          ...
          <label className="mt-2 flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={settings.categories == null}
              onChange={(e) => setSettings((s) => ({ ...s, categories: e.target.checked ? null : [] }))}
              disabled={disabled}
            />
            All categories
          </label>
          {settings.categories != null ? (
            <div className="mt-2 grid grid-cols-2 gap-1.5 pl-6 sm:grid-cols-3">
              {VENDOR_DISPATCH_CATEGORY_OPTIONS.map((opt) => (
                <label key={opt.value} className="flex items-center gap-2 text-sm">
                  <input
                    type="checkbox"
                    checked={settings.categories?.includes(opt.value) ?? false}
        ...
        <div>
          <p className="text-xs font-semibold text-muted">Notify me</p>
          <div className="mt-2 flex flex-wrap gap-3">
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={settings.notify.push}
                onChange={(e) => setSettings((s) => ({ ...s, notify: { ...s.notify, push: e.target.checked } }))}
                disabled={disabled}
              />
              Push
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(e) => setSettings((s) => ({ ...s, notify: { ...s.notify, sms: e.target.checked } }))}
                disabled={disabled}
              />
              Text me
            </label>
        ...
        <Button
          ...
          data-attr="vendor-dispatch-settings-save"
          onClick={() => void save()}
          disabled={disabled}
        >
          {busy ? "Saving…" : "Save"}
