...
export function PortalTopbar({ definition }: { definition: PortalDefinition }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border glass-nav bg-background/88">
      <div className="flex items-center justify-between gap-3 px-4 py-3 lg:px-8">
        <div className="min-w-0">
          <p className="truncate text-[11px] font-semibold uppercase tracking-[0.16em] text-muted">Signed in</p>
          <p className="truncate text-sm font-medium text-foreground">{definition.title} · {formatPath(pathname)}</p>
        </div>
