 ...
 * This is a subscription upsell, so the whole banner is hidden on native iOS
...
export function ManagerPlanBanner({ planHref = MANAGER_PLAN_PORTAL_URL }: { planHref?: string }) {
  return (
    <div className="native-hide shrink-0 border-b border-amber-300 bg-[#fffbeb] px-[max(1rem,env(safe-area-inset-left,0px))] py-2.5 pe-[max(1rem,env(safe-area-inset-right,0px))] text-center text-xs leading-snug text-amber-950 sm:text-sm lg:px-8">
